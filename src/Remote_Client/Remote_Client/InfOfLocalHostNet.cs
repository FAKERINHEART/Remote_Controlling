...
namespace Remote_Client
{
    public partial class InfOfLocalHostNet : Form
    {
        ...
        {          
            this.InfOfLocalHostNetLabel.Text = "本机共有网络适配器" + ((Remote_Client)this.Owner).GetNIS.Length.ToString() + "个:" + "\n";
            int index1 = 0;
            foreach (NetworkInterface NI in ((Remote_Client)this.Owner).GetNIS)
            {
